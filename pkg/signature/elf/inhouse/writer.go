...
func saveELFSignature(f *elfFile, payload []byte) ([]byte, error) {
	note := createELFNote(f.byteOrder, signatureNoteName, payload, signatureNoteType)
	...
	for i, sh := range f.shdrs {
		...
		sec := keptSection{oldIdx: i, hdr: sh, name: name}
		if i < insertAt && name != shstrtabSectionName {
	...
	for _, sec := range after {
		if sec.hdr.Flags&shfAlloc != 0 {
			return nil, fmt.Errorf("refusing to relocate SHF_ALLOC section %q without updating program headers", sec.name)
		}
	}
	if shstrtabSec.hdr.Flags&shfAlloc != 0 {
		return nil, fmt.Errorf("refusing to relocate SHF_ALLOC section %q without updating program headers", shstrtabSec.name)
	}
	...
	for i := range after {
		var err error
		out, err = alignOutput(out, after[i].hdr.Addralign)
		if err != nil {
			return nil, fmt.Errorf("align section %q: %w", after[i].name, err)
		}
	...
	var err error
	out, err = alignOutput(out, shstrtabSec.hdr.Addralign)
	if err != nil {
		return nil, fmt.Errorf("align section %q: %w", shstrtabSec.name, err)
	}
...
func alignOutput(out []byte, alignment uint64) ([]byte, error) {
	if alignment <= 1 {
		return out, nil
	}

	remainder := uint64(len(out)) % alignment
	if remainder == 0 {
		return out, nil
	}
	padding := alignment - remainder
	maxInt := int(^uint(0) >> 1)
	if padding > uint64(maxInt-len(out)) {
		return nil, fmt.Errorf("alignment padding overflows output size")
	}
	return append(out, make([]byte, int(padding))...), nil
}
...
func getELFSignature(f *elfFile) ([]byte, error) {
	...
	note, err := parseELFNote(f.byteOrder, data)
	if err != nil {

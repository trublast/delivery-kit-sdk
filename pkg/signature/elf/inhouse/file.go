...
func (f *elfFile) parseProgramHeaders() error {
	...
	if f.ehdr.Phnum == pnXNum {
		...
		if f.ehdr.Shoff == 0 || !rangeWithinFile(f.ehdr.Shoff, 64, len(f.data)) {
	...
	if phnum > maxELFProgramHeaders {
		return fmt.Errorf("program header count %d exceeds limit %d", phnum, maxELFProgramHeaders)
	}
	if f.ehdr.Phoff == 0 {
	...
	if !rangeWithinFile(f.ehdr.Phoff, phnum*56, len(f.data)) {
...
func (f *elfFile) parseSectionHeaders() error {
	...
	if f.ehdr.Shnum == 0 {
		...
		if shoff == 0 || !rangeWithinFile(shoff, 64, len(f.data)) {
	...
	if shnum > maxELFSections {
		return fmt.Errorf("section count %d exceeds limit %d", shnum, maxELFSections)
	}
	if !rangeWithinFile(shoff, shnum*64, len(f.data)) {
	...
	if !rangeWithinFile(strtab.Offset, strtab.Size, len(f.data)) {
